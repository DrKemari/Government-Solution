...
Page 69153 "Approved Salary Voucher Temp"
{
    Editable = false;
    PageType = Card;
    ...
    actions
    {
        area(navigation)
        {
            group(ActionGroup16)
            {
                ...
                action("Post Payroll (Project)")
                {
                    ApplicationArea = Basic;
                    Caption = 'Post Payroll ((Project)';
                    Image = Post;
                    Promoted = true;
                    PromotedCategory = New;
                    PromotedIsBig = true;
                    ToolTip = 'Executes the Post Payroll ((Project) action.';
                    trigger OnAction()
                    begin
                        ...
                        Report.Run(69409, true, true, PayPeriod);
                ...
                action("Post Payroll")
                {
                    ...
                    begin
                        ...
                        emp.SetFilter(emp."Payroll Period", Format(Rec."Payroll Period"));
                        Report.Run(69408, true, true, emp);

                        DocNumber := 'TEMPPAYROLL' + Format(Date2dmy(Rec."Payroll Period", 2)) + Format(Date2dmy(Rec."Payroll Period", 3));
                        GLEntry.Reset;
                        GLEntry.SetRange(GLEntry."Document No.", DocNumber);
                        GLEntry.SetRange(GLEntry.Reversed, false);
                        if GLEntry.FindFirst then begin
