...
Page 85432 "Chief Examiner Comment Card"
{
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            part(Control15; "Chief Exam Specific Comments")
            {
                SubPageLink = "Document No." = field("Document No.");
                ApplicationArea = Basic;
            }
            part(Control14; "Chief General Comments")
            {
                SubPageLink = "Document No." = field("Document No.");
                ApplicationArea = Basic;
            }
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Suggest Reviewed Questions")
            {
                ApplicationArea = Basic;
                ...
                PromotedIsBig = true;
                ToolTip = 'Executes the Suggest Reviewed Questions action.';
                trigger OnAction()
                var
                ...
                begin
                    if Confirm(TXT001, true, Rec."Paper Code") = true then begin
                        SettingandModeration.SuggestReviewedQuestions(Rec);
                    end
                end;
            }
            action(Approve)
    ...
    begin
        Rec."Document Type" := Rec."document type"::"Chief Examiner"
    end;
