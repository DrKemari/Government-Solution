        ...
        area(navigation)
        {
            group("Request for EOI")
            {
                ...
                action("Attach Memo")
                {
                    ...
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                    ToolTip = 'Executes the Attach Justification Memo action.';
                    // trigger OnAction()
                    // var
                    //     filename: Text;
                ...
                action("View Memo")
                {
                    ...
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                    ToolTip = 'Executes the View Memo action.';
                    trigger OnAction()
                    begin
                ...
                action("Generate Purchase Order")
                {
                    ...
                    begin
                        ...
                        if not Rec."Generate Multiple LPO" then begin
                            ...
                            // MESSAGE('%1',PurchReq.Status);

                            PurchReq.Reset;
                            PurchReq.SetRange("No.", Rec."Requisition No");
                            if PurchReq.FindSet then
                                //MESSAGE('%1',PurchReq.Status);
                                if PurchReq.Status = PurchReq.Status::"In Progress" then begin
