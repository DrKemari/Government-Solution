...
Page 69327 "Generate Applicant Requests"
{
    ...
    SourceTableView = sorting("Company Name", "Company No.", Type, Name)
                      where("Portal Request" = const(true), Generated = const(true));
    UsageCategory = Lists;
    ...
    actions
    {
        area(navigation)
        {
            ...
            group(Tasks)
            {
                ...
                action("Open Oppo&rtunities")
                {
                    ...
                    RunPageLink = "Contact Company No." = field("Company No."), "Contact No." = filter(<> ''), "Contact No." = field(filter("Lookup Contact No.")), Status = filter("Not Started" | "In Progress");
                    RunPageView = sorting("Contact Company No.", "Contact No.");
                    Scope = Repeater;
            ...
            group(History)
            {
                ...
                action("Closed Oppo&rtunities")
                {
                    ...
                    Image = OpportunityList;
                    RunObject = Page "Opportunity List";
                    RunPageLink = "Contact Company No." = field("Company No."), "Contact No." = filter(<> ''), "Contact No." = field(filter("Lookup Contact No.")), Status = filter(Won | Lost);
                    RunPageView = sorting("Contact Company No.", "Contact No.");
        ...
        area(processing)
        {
            ...
            action("Create Opportunity")
            {
                ...
                RunPageLink = "Contact No." = field("No."), "Contact Company No." = field("Company No.");
                RunPageMode = Create;
