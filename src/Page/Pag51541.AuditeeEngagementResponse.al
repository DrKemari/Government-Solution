...
Page 51541 "Auditee Engagement Response"
{
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            ...
            part(Control44; "Engagement Auditee Team")
            {
    ...
    actions
    {
        area(processing)
        {
            action("Request Re-Update")
            {
                ...
                PromotedIsBig = true;
                ToolTip = 'Executes the Request Re-Update action.';
                trigger OnAction()
                var
            ...
            action("Initiate SAR")
            {
                ...
                PromotedCategory = Process;
                PromotedIsBig = true;
                ToolTip = 'Executes the Publish SAR action.';
                trigger OnAction()
                var
                    TXT001: label 'Initiate SAR Request for %1';
                begin
                    Rec.TestField("Expected Submission End Date");
                    if Confirm(TXT001, true, Rec.Description) = true then begin
                        //   AccreditationQualityAudit.NotifyInstitutionOnSAR(Rec);
                        Rec.Published := true;
                        Rec."Published By" := UserId;
                        Rec."Published On" := Today;
                    end
        ...
        area(creation)
        {
            ...
            action(Print)
            {
                ...
                begin
                    ...
                    Report.Run(51303, true, false, Rec);
                end;
            }
        }
    }
