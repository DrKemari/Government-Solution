...
Page 85253 "Manual Student Application"
{
    ...
    layout
    {
        ...
        area(factboxes)
        {
            ...
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = All;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(85029),
                              "No." = FIELD("No.");
            }
    ...
    actions
    {
        area(creation)
        {
                    ...
                    if Rec."Institution Code" = '' then begin
                        if Confirm(TXT001, true, Rec."ID Number/Passport No.") = true then begin
                            if Rec."Student No." = '' then begin
                                Examination.ApplicationOnlyOnboardCustomer(Rec);
                            end;
            ...
            action("Send A Pro-Forma Invoice")
            {
                ApplicationArea = Basic;
                Image = OpenJournal;
                Promoted = true;
                ...
                PromotedIsBig = true;
                Visible = false;
                ToolTip = 'Executes the Send A Pro-Forma Invoice action.';
                trigger OnAction()
                begin

                    Rec.TestField("Student No.");
                    Rec.TestField(Email);
                    if Rec."Application Invoice" <> '' then begin
                        Error('The application invoice %1,is already created', Rec."Application Invoice");
                    end;
                    if Confirm('Are you sure you want to post a sales invoice for this application') = true then begin
                        Examination.ApplicationCreateInvoice(Rec);
                        Message('Invoice %1 has been succesfully sent to %2', Rec."Application Invoice", Rec.Email)
                    end
                end;
            }
            action("Pay Now")
