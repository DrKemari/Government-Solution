report 85048 "Exemption Receipt"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Exemption Receipt.rdlc';

    dataset
    {
        dataitem("Student Processing Header";"Student Processing Header")
        {
            DataItemTableView = WHERE("Document Type"=FILTER(Exemption));
            ...
            column(RegistrationReceiptNo;"Student Processing Header"."Registration Receipt No.")
            {
            }
            ...
            column(ExemptionReceiptNo;ReceiptNo)
            {
            }
            ...
            dataitem(StudentProcessingLines;"Exemption Lines")
            {
                DataItemLink = "Header No."=FIELD("No.");
                column(BookingHeaderNo_StudentProcessingLines;StudentProcessingLines."Header No.")
                {
                }
                column(LineNo_StudentProcessingLines;StudentProcessingLines."Line No.")
                {
                }
                column(CourseId_StudentProcessingLines;StudentProcessingLines."Course Id")
                {
                }
                column(Section_StudentProcessingLines;StudentProcessingLines."No.")
                {
                }
                column(Part_StudentProcessingLines;StudentProcessingLines."No.")
                {
                }
                column(Paper_StudentProcessingLines;StudentProcessingLines."No.")
                {
                }
                column(Description_StudentProcessingLines;StudentProcessingLines.Name)
                {
                }
                column(CurrencyCode_StudentProcessingLines;StudentProcessingLines."Currency Code")
                {
                }
                column(Amount_StudentProcessingLines;StudentProcessingLines.Amount)
                {
                }
                column(AmountLCY_StudentProcessingLines;StudentProcessingLines."Amount LCY")
                {
                }
            }
            ...
            begin
                ...
                ReceiptsHeader1.SetRange("No.","Student Processing Header"."Exemption Receipt 6No.");
                if ReceiptsHeader1.FindLast then begin
                  TodayDate:=ReceiptsHeader1."Posting Date";
                  BankName:=ReceiptsHeader1."Bank Name";
                  Paymode:=ReceiptsHeader1."Pay Mode";
                end else
                TodayDate:=Today;

                if "Student Processing Header"."Exemption Receipt 6No."='' then
                  ReceiptNo:="Student Processing Header"."Payment Reference No." else
                  ReceiptNo:="Student Processing Header"."Exemption Receipt 6No.";
            end;
        }
    }
