report 70014 "Annual Procurement Plan G&S"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Annual Procurement Plan G&S.rdlc';
    ...
    dataset
    {
        dataitem("Procurement Header";"Procurement Header")
        {
            ...
            column(TenderCommittee;TenderCommittee)
            {
            }
            ...
            dataitem("Procurement Plan1";"Procurement Plan1")
            {
                ...
                DataItemTableView = WHERE("Procurement Type"=FILTER(Goods));
            ...
            begin
                ProcRequest.Reset;
                ProcRequest.SetRange("Procurement Plan No", No);
                if ProcRequest.FindSet then
                  "TenderCommitteeNo.":=ProcRequest."Tender Committee";
                   if TenderCApp.Get("TenderCommitteeNo.") then
                      TenderCommittee:=TenderCApp."Committee Name";
                /*IF CompanyInfo.GET(CompanyInfo.Name) THEN
                  CompanyName:=CompanyInfo.Name;*/
                
                /*IF WorkType.GET("Employee registration lines"."Registration Code") THEN
    ...
    var
        ...
        TenderCommittee: Text[100];
        "TenderCommitteeNo.": Code[150];
        TenderCApp: Record "Tender Commitee Appointment1";
}
