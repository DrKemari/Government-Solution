...
Codeunit 50007 "Requisition Management"
{
    ...
    begin
        with PurchaseHeader do begin
         ...
         Committment.LPOCommittment(PurchaseHeader);
          ...
          //Machira Added to Copy documents attached from the links)

        PurchHeader.Reset;
        PurchHeader.SetRange("No.",RequisitionLine."Document No.");
        if PurchHeader.FindFirst then begin
         IDEntry:=PurchHeader.RecordId;
        RecordLink.Reset;
        ...
        if RecordLink.FindSet then begin
          repeat
            ...
            RecordLink1."Record ID":=PurchaseHeader.RecordId;
    ...
    begin
        with PurchaseHeader do begin
          if ((LastVendorProcessed = RequisitionLine."Vendor(Dealer)") and (LastVendorProcessed <> '')) then begin
            LineNo := LineNo + 10000;
            Get(DocumentType,DocumentNo);
