#pragma warning disable AA0005, AA0008, AA0018, AA0021, AA0072, AA0137, AA0201, AA0206, AA0218, AA0228, AL0254, AL0424, AS0011, AW0006 // ForNAV settings
Table 85033 "Center Booking Lines"
{

    fields
    {
        field(1;"Header No.";Code[30])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Center Booking Header";
        }
        field(2;"Line No.";Integer)
        {
            AutoIncrement = true;
            DataClassification = ToBeClassified;
        }
        field(3;"Student Reg No.";Code[30])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Exam Booking Entries"."Student Reg No.";

            trigger OnValidate()
            begin
                ExaminationAccount.Reset;
                ExaminationAccount.SetRange("Registration No.","Student Reg No.");
                if ExaminationAccount.FindSet then begin
                  "Student Name":=ExaminationAccount."First Name"+' '+ExaminationAccount."Middle Name"+' '+ExaminationAccount.Surname;
                  "Course Id":=ExaminationAccount."Course ID";
                  "Course Description":=ExaminationAccount."Course Description";

                end;
            end;
        }
        field(4;"Student Name";Text[150])
        {
            DataClassification = ToBeClassified;
        }
        field(5;"Course Id";Code[30])
        {
            DataClassification = ToBeClassified;

            trigger OnValidate()
            begin
                if Courses.Get("Course Id") then begin
                  "Course Description":=Courses.Description;
                end
            end;
        }
        field(6;"Course Description";Text[100])
        {
            DataClassification = ToBeClassified;
        }
        field(7;Region;Code[30])
        {
            DataClassification = ToBeClassified;
        }
        field(8;Zone;Code[30])
        {
            DataClassification = ToBeClassified;
        }
        field(9;"Exam Center Code";Code[30])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Examination Centres";

            trigger OnValidate()
            begin
                if ExamCenter.Get("Exam Center Code") then
                  "Center Name":=ExamCenter.Name;
            end;
        }
        field(10;"Center Name";Text[100])
        {
            DataClassification = ToBeClassified;
        }
        field(11;"Sitting Cycle";Code[30])
        {
            DataClassification = ToBeClassified;
        }
        field(12;Year;Code[10])
        {
            DataClassification = ToBeClassified;
        }
        field(13;"Maximum Capacity";Integer)
        {
            DataClassification = ToBeClassified;
        }
        field(14;"Available Capacity";Integer)
        {
            DataClassification = ToBeClassified;
        }
        field(15;"Subject Code";Code[30])
        {
            DataClassification = ToBeClassified;
            TableRelation = Papers;

            trigger OnValidate()
            begin
                if Papers.Get("Subject Code") then
                  "Subject Name":=Papers.Description;
            end;
        }
        field(16;"Subject Name";Text[50])
        {
            DataClassification = ToBeClassified;
        }
        field(17;Type;Option)
        {
            DataClassification = ToBeClassified;
            OptionCaption = 'Allocation,Re-Allocation,Cancellation';
            OptionMembers = Allocation,"Re-Allocation",Cancellation;
        }
        field(18;"Last Modified Date Time";DateTime)
        {
            Caption = 'Last Modified Date Time';
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(19;"Last Date Modified";Date)
        {
            Caption = 'Last Date Modified';
            DataClassification = ToBeClassified;
            Editable = false;
        }
        field(20;Moved;Boolean)
        {
            DataClassification = ToBeClassified;
        }
    }

    keys
    {
        key(Key1;"Header No.","Student Reg No.",Type,"Subject Code")
        {
            Clustered = true;
        }
    }

    fieldgroups
    {
    }

    trigger OnDelete()
    var
        TXT001: label 'You are not allowed to delete an entry that is already moved';
    begin
        if Moved=true then
          Error(TXT001);
    end;

    trigger OnModify()
    begin
        "Last Date Modified":=Today;
        "Last Modified Date Time":=CurrentDatetime;
    end;

    var
        ExamCenter: Record "Examination Centres";
        CenterCourseCapacity: Record "Center Course Capacity";
        ExaminationAccount: Record "Examination Account";
        Papers: Record Papers;
        Courses: Record Courses;
}

