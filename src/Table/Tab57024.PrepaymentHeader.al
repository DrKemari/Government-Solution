    ...
    fields
    {
        ...
        field(18;"Paying Bank Account";Code[20])
        {
            TableRelation = if ("Payment Type"=const("Payment Voucher")) "Bank Account" where (Type=const(Bank))
                            ...
                            else if ("Payment Type"=const("Salary Advance")) "Bank Account" where (Type=filter(Bank|"Petty Cash"))
                            else if ("Payment Type"=const("Standing Imprest Surrender")) "Bank Account" where (Type=filter(Bank|"Petty Cash"));
        }
