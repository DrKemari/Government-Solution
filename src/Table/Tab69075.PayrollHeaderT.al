...
Table 69075 "Payroll HeaderT"
{
    DrillDownPageID = "Payroll Processing List Temp";
    LookupPageID = "Payroll Processing List Temp";
    ...
    fields
    {
        field(1;"No.";Code[30])
        {
            ...
            begin
                ...
                NoSetup.Get;
                NoSeriesMgt.TestManual(NoSetup."Temp Salary Voucher Nos.");
                "No. Series" := '';
                end;
            end;
        }
        ...
        {
            Editable = true;
            TableRelation = "Payroll PeriodXT"."Starting Date" where (Closed=const(false));
        ...
        {
            CalcFormula = sum("Payroll LinesT"."Gross Pay" where ("Payroll Header"=field("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(8;"Net Pay";Decimal)
        {
            CalcFormula = sum("Payroll LinesT".Netpay where ("Payroll Header"=field("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = sum("Payroll LinesT"."Total Deduction" where ("Payroll Header"=field("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
    ...
    begin
        ...
        if "No." = '' then begin
          NoSetup.Get;
          NoSetup.TestField(NoSetup."Temp Salary Voucher Nos.");
          NoSeriesMgt.InitSeries(NoSetup."Temp Salary Voucher Nos.",xRec."No. Series",0D,"No.","No. Series");
        end;
